<foreignObject >
    <style>
        h1,h2,h3,h4,h5,h6{
            font-weight: normal;
            font-family: "Calibri";
            color: rgba(255,255,255,.6);
        }
        h6{
            margin: 0;
        }
        li {
            font-family: "Calibri";
        }
        .steps li{
            list-style-type: decimal-leading-zero;
            text-indent: 5px;
        }
        blockquote{
            font-family: "Calibri";
            color: rgba(255,255,255,.6);
            border-left: none;
            /*border-left:8px solid tomato;*/
        }
    </style>
</foreignObject>
<h1><b>TELEGRAM BOT USING PHP</b></h1>
<h6>Things required :</h6>
<blockquote>
    <ul style="list-style-type: none;padding: 0">
        <li>PHP V7.4.3 +</li>
        <li>Composer ( <a href="https://getcomposer.org/download/"> click here </a> for guide  )</li>
        <li>Ngrok ( Download from <a href="https://ngrok.com/download">here</a>, and read docs for more information)</li>
        <li>Lot of internet XD</li>
        <li><h6>Nothing else required you're ready to rock</h6></li>
    </ul>
</blockquote>
<h4>Steps for creating bot in telegram : </h4>
<blockquote>
    <ul class="steps">
        <li>Open telegram and search for bot father</li>
        <li>type /newbot</li>
        <li>Add bot name and username</li>
        <li>list your bot and choose your bot</li>
        <li>Click on API Token</li>
        <li>Copy the token and save it some were</li>
        <li>More information will be available <a href="https://kri55h.blogspot.com/2023/05/telegram-bot.html">here</a> soon  </li>
    </ul>
</blockquote>

[//]: # (<h4>Steps for creating php project :- </h4>)

[//]: # (<blockquote>)

[//]: # (    <ul class="steps">)

[//]: # (        <li></li>)

[//]: # (        <li>type /newbot</li>)

[//]: # (        <li>Add bot name and username</li>)

[//]: # (    </ul>)

[//]: # (</blockquote>)